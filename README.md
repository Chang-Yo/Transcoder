...
| 文件                                                                                          | 大小   | 说明                    |
| --------------------------------------------------------------------------------------------- | ------ | ----------------------- |
| [transcoder-v0.5.2-windows.zip](../../releases/download/v0.5.2/transcoder-v0.5.2-windows.zip) | ~72 MB | **推荐** — 解压即可运行 |

**系统要求：** Windows 10/11 (64-bit)

**使用方法：** 下载后解压 zip 文件，双击 `transcoder.exe` 即可运行。优先使用系统已安装的 FFmpeg（如果 PATH 中可用），否则使用内置的 `ffmpeg/` 文件夹中的版本。
...
- **文件大小估算** — 转码前预估输出文件大小
- **拖拽操作** — 支持拖拽视频文件或文件夹
- **FFmpeg 灵活检测** — 优先使用系统 FFmpeg，内置版本作为备选

## 使用方法

### 快速开始

1. **启动应用** — 双击运行 `transcoder.exe`
2. **添加视频** — 点击左侧边栏的 "+ Add Files" 按钮，或直接拖拽视频文件到窗口
   - 支持多选或拖拽多个文件
   - 支持拖拽文件夹，自动识别其中的视频文件
3. **选择格式** — 在左侧边栏点击选择输出格式预设
...
4. **确认输出** — 在左侧边栏查看预估文件大小，确认或修改输出目录
5. **(可选) 裁剪片段** — 展开文件卡片，勾选 "Enable time range" 可只转码选中的时间段
   - 拖动滑块或输入时间码（HH:MM:SS）设置起止时间
   - 适用于提取视频中的特定片段
6. **开始转码** — 点击左侧边栏的 "Start" 按钮
7. **等待完成** — 查看底部状态栏的实时进度，转码完成后导入 PR/AE 使用

### 界面布局

![界面展示](/public/Interface.png)
...
A: 下载 zip 文件后解压，双击 `transcoder.exe` 即可运行。无需安装，FFmpeg 已内置在 `ffmpeg/` 文件夹中。
...
A: 应用会优先使用系统 PATH 中的 FFmpeg（如果存在），否则使用内置版本。这样可以利用系统更新的 FFmpeg 版本，同时保证在没有安装 FFmpeg 的系统上也能正常运行。
